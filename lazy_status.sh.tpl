#!/usr/bin/env bash
# Status script for lazy_bazel_env
...
marker_pattern="_bazel_env_marker_{{name}}"
if compgen -c | grep -q "$marker_pattern" 2>/dev/null; then
    ...
    if ! grep -qE '[[:<:]]lazy_bazel_env[[:>:]]' .envrc 2>/dev/null; then
      echo ""
      if [[ -f .envrc ]]; then
...
PATH_add {{bin_dir}}
if [[ ! -d {{bin_dir}} ]]; then
  log_error "ERROR[lazy_bazel_env]: Run 'bazel run {{label}}' to regenerate {{bin_dir}}"
fi
ENVRC_EOF
      step_num=$((step_num + 1))
    fi
...
if [[ "$outer_shell_name" == *zsh* ]]; then
  echo "Remember to run 'rehash' in zsh to update the locations of binaries on the PATH."
elif [[ "$outer_shell_name" == *bash* ]]; then
  echo "Remember to run 'hash -r' in bash to update the locations of binaries on the PATH."
fi
